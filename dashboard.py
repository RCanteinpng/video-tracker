...
# Exact CSS for the Iman Gadzhi replica look
st.markdown("""
    <style>
...
try:
    # 1. Load and Clean Data (Removes 'nan' rows automatically)
    ...
    with col_1:
        ...
        # Using 'hqdefault' as it is the most reliable image format
        ...
        st.markdown('<div class="metric-row"><span class="metric-value">5.85%</span></div>', unsafe_allow_html=True)
    ...
    with col_2:
        ...
        st.markdown('<div class="metric-row"><span class="metric-value">4.46%</span></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
